...
<Tip>When there’s limited space or no default selection, consider using a select <Link href="/?path=/docs/components-dropdown--overview">Dropdown</Link> instead.</Tip>
  ...
  rules={[
    {
      ...
      negative: {
        ...
        description:<>Don't use <Link href="/?path=/docs/components-checkbox--overview" size={Link.sizes.MEDIUM}>checkboxes</Link> when only one item can be selected from a list. Use radio buttons instead.</>
      }
    },
    {
      positive: {
      ...
      negative: {
        ...
        description:<>Don’t leave all radio buttons unselcted. If you can’t offer a smart default, consider using the <Link href="/?path=/docs/components-dropdown--overview" size={Link.sizes.MEDIUM}>dropdown component.</Link> </>
      }
    }
  ]}
/>
...
<Canvas>
  <Story name="Radio button in items list">
    <div className="monday-style-radio-buttons_wrapper-column">
      ...
      <RadioButton text="Inbox updates" checked />
